...
export default function Home({ currentUser }) {
  ...
  useEffect(() => {
    const fetchWatchlist = async () => {
      ...
      try {
        const { data, error } = await supabase.from('watchlists').select('movie_id').eq('user_id', currentUser.id).eq('domain', 'GL');
        if (error) throw error;
  ...
  const toggleWatchlist = async (e, movieId) => {
    ...
    try {
      ...
      if (isSaved) {
        const { error } = await supabase.from('watchlists').delete().eq('user_id', currentUser.id).eq('movie_id', movieId).eq('domain', 'GL');
        if (error) throw error;
        setWatchlist(prev => prev.filter(id => id !== movieId));
        showToast('ลบออกจากรายการโปรดแล้ว', 'success');
      } else {
        const { error } = await supabase.from('watchlists').insert([{ user_id: currentUser.id, movie_id: movieId, domain: 'GL' }]);
        if (error) throw error;
  ...
  const submitReview = async (e) => {
    ...
    try {
      const { error } = await supabase.from('reviews').insert([{
        user_id: currentUser.id, movie_id: selectedMovie.id, domain: 'GL', rating: newRating, review_text: newReview
      }]);
      ...
      const { data } = await supabase.from('reviews').select('*, profiles(username)').eq('movie_id', selectedMovie.id).eq('domain', 'GL').order('created_at', { ascending: false });
      if (data) setReviews(data);
    } catch (error) {
      showToast('เกิดข้อผิดพลาด: ' + error.message, 'error');
      console.error(error);
  ...
  return (
    <div className="flex flex-col min-h-screen font-prompt">
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="relative w-full aspect-[16/9] md:aspect-[21/9] min-h-[350px] md:min-h-[400px] rounded-2xl overflow-hidden mb-12 shadow-2xl bg-black group">
            {heroMovies.map((movie, index) => (
              ...
              >
                ...
                <div className="absolute bottom-6 left-4 right-4 md:top-1/2 md:left-12 md:-translate-y-1/2 md:w-1/2 z-20 text-white flex flex-col md:block items-center md:items-start text-center md:text-left">
                  <span className="inline-block bg-pink-500 text-white px-3 py-1 rounded text-xs font-bold mb-3 uppercase tracking-widest">{movie.status || 'ON AIR'}</span>
                  <h1 className="font-serif text-2xl md:text-4xl font-bold mb-2 leading-tight drop-shadow-md line-clamp-2">{movie.title}</h1>
                  <p className="text-pink-400 font-medium mb-2 md:mb-4 text-sm md:text-base">{movie.genre} {movie.rating ? `• ${movie.rating}` : ''}</p>
                  ...
                  <button className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-2.5 rounded-full font-bold transition-all duration-300 flex items-center gap-2 shadow-[0_4px_15px_rgba(236,72,153,0.4)] hover:-translate-y-0.5 hover:scale-105">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 md:left-auto md:right-8 md:translate-x-0 flex gap-2 z-20">
              {heroMovies.map((_, index) => (
                <button 
                  key={index} 
                  className={`h-2.5 rounded-full transition-all duration-300 ${index === currentSlide ? 'w-6 bg-pink-500' : 'w-2.5 bg-white/40 hover:bg-white/60'}`}
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 mb-12 shadow-lg">
          <div className="relative w-full max-w-2xl mx-auto mb-6">
            ...
            <input 
              ...
              className="w-full bg-black/20 border border-white/10 text-white rounded-xl pl-12 pr-4 py-3 backdrop-blur-md focus:outline-none focus:bg-black/40 focus:border-pink-500 focus:ring-1 focus:ring-pink-500/30 transition-all duration-300 placeholder:text-gray-500"
            />
          </div>
          
          <div className="flex flex-wrap justify-center gap-4 w-full mb-6">
            <select value={selectedGenre} onChange={(e) => setSelectedGenre(e.target.value)} className="flex-1 min-w-[150px] max-w-[250px] bg-black/20 border border-white/10 text-gray-200 rounded-xl px-4 py-2.5 backdrop-blur-md focus:outline-none focus:border-pink-500 transition-colors [&>option]:bg-gray-900 cursor-pointer appearance-none">
            ...
            <select value={selectedPlatform} onChange={(e) => setSelectedPlatform(e.target.value)} className="flex-1 min-w-[150px] max-w-[250px] bg-black/20 border border-white/10 text-gray-200 rounded-xl px-4 py-2.5 backdrop-blur-md focus:outline-none focus:border-pink-500 transition-colors [&>option]:bg-gray-900 cursor-pointer appearance-none">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="flex-1 min-w-[150px] max-w-[250px] bg-black/20 border border-white/10 text-gray-200 rounded-xl px-4 py-2.5 backdrop-blur-md focus:outline-none focus:border-pink-500 transition-colors [&>option]:bg-gray-900 cursor-pointer appearance-none">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {statusOptions.map(stat => (
              <button 
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 border ${selectedStatus === stat.id ? 'bg-pink-500/20 text-pink-400 border-pink-500/50' : 'bg-black/20 text-gray-400 border-white/10 hover:border-white/30 hover:text-white'}`}
              >
                {stat.label}
              </button>
            ))}
        ...
        <div id="movie-section" className="mb-6 pl-4 border-l-4 border-pink-500">
          <h2 className="text-2xl md:text-3xl font-bold text-white">GL Now Showing</h2>
        </div>

        {/* MOVIE GRID */}
        {loading ? (
          <div className="text-center py-20 text-pink-500 font-bold text-xl">{t('home.loading') || 'กำลังโหลด...'}</div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {paginatedMovies.length > 0 ? paginatedMovies.map(movie => (
                <div key={movie.id} className="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden cursor-pointer flex flex-col transition-all duration-300 hover:-translate-y-1.5 hover:border-pink-500/50 hover:shadow-[0_15px_40px_-10px_rgba(236,72,153,0.3)]" onClick={() => handleMovieClick(movie)}>
                  <div className="relative w-full aspect-[16/9] overflow-hidden bg-black">
                    ...
                    <button 
                      className={`absolute top-2 right-2 w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-md transition-all duration-300 shadow-lg ${watchlist.includes(movie.id) ? 'bg-pink-500 border-none text-white' : 'bg-black/50 border border-white/20 text-white hover:bg-pink-500 hover:border-pink-500 hover:scale-110'}`}
                  ...
                  <div className="p-4 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto w-full bg-pink-500/10 text-pink-400 border border-white/5 py-2.5 rounded-lg font-semibold text-center text-sm transition-all duration-300 group-hover:bg-pink-500 group-hover:text-white group-hover:border-pink-500">
                      More Details
                    </div>
                  </div>
                </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 mt-12 mb-8 flex-wrap">
                <button 
                  className="bg-white/5 border border-white/10 text-white px-5 py-2.5 rounded-lg cursor-pointer transition-all duration-300 hover:bg-pink-500 hover:border-pink-500 disabled:opacity-40 disabled:cursor-not-allowed font-medium"
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex justify-center items-center p-4 animate-in fade-in duration-200" onClick={() => setSelectedMovie(null)}>
          <div className="bg-gray-900/60 backdrop-blur-2xl border border-white/10 w-full max-w-4xl rounded-2xl flex flex-col max-h-[90vh] shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-between items-center p-5 border-b border-white/10 bg-black/20">
              ...
              <button className="bg-white/5 border border-white/10 rounded-full text-gray-400 w-9 h-9 flex items-center justify-center transition-all duration-300 hover:text-white hover:bg-pink-500 hover:border-pink-500 hover:rotate-90" onClick={() => setSelectedMovie(null)}>
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-grow custom-scrollbar">
                  ...
                  <>
                    ...
                    {selectedMovie.youtube_url && (
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                        <a href={selectedMovie.youtube_url} target="_blank" rel="noopener noreferrer" className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-3 rounded-full font-bold transition-all duration-300 flex items-center gap-2 shadow-[0_4px_15px_rgba(236,72,153,0.5)] hover:scale-105">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <h1 className="text-2xl md:text-3xl font-bold text-pink-500 m-0 leading-tight">{selectedMovie.title || t('home.no_title')}</h1>
                <button 
                  className={`shrink-0 px-4 py-2 rounded-full flex items-center gap-2 font-medium transition-all duration-300 border ${watchlist.includes(selectedMovie.id) ? 'bg-pink-500 border-pink-500 text-white shadow-[0_0_15px_rgba(236,72,153,0.3)]' : 'bg-transparent border-pink-500/50 text-pink-400 hover:bg-pink-500/10'}`}
              ...
              <h4 className="text-lg font-bold text-white mb-3 flex items-center gap-2"><Info size={18} className="text-pink-500"/> {t('home.synopsis') || 'Synopsis'}</h4>
              ...
              <div className="mt-10 border-t border-white/10 pt-8">
                <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><MessageSquare size={20} className="text-pink-500"/> คะแนนและรีวิวจากผู้ชม</h3>
                ...
                {currentUser ? (
                  <form onSubmit={submitReview} className="bg-black/20 p-6 rounded-xl border border-white/5 mb-8">
                    ...
                    <textarea 
                      className="w-full bg-black/40 border border-white/10 text-white rounded-xl p-4 focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500/30 transition-all duration-300 resize-none placeholder:text-gray-600" 
                      rows="3" 
                    ...
                    <div className="flex justify-end mt-4">
                      <button type="submit" className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-2.5 rounded-lg font-bold transition-all duration-300 shadow-[0_4px_12px_rgba(236,72,153,0.3)]" disabled={isSubmittingReview}>
                        {isSubmittingReview ? 'กำลังส่ง...' : 'ส่งรีวิว'}
                      </button>
                    </div>
                  </form>
                ) : (
                  <div className="bg-pink-500/10 border border-pink-500/20 p-4 rounded-xl text-center text-pink-400 mb-8">
