...
const Schedule = () => {
  ...
  const translateDay = (dayStr) => {
    if (dayStr === 'N/A') return t('schedule.unknown_day');
    const dayMap = {
      'วันจันทร์': t('day.mon'), 'วันอังคาร': t('day.tue'), 'วันพุธ': t('day.wed'),
      'วันพฤหัสบดี': t('day.thu'), 'วันศุกร์': t('day.fri'), 'วันเสาร์': t('day.sat'), 'วันอาทิตย์': t('day.sun'),
    };
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      const { data, error } = await supabase.from('movies').select('*').order('air_time', { ascending: true });
  ...
  return (
    <div>
      <div className="text-center" style={{ margin: '3rem 0 2rem' }}>
        <span className="gl-subtitle">{t('schedule.subtitle')}</span>
        <h1 className="gl-title">{t('schedule.title')}</h1>
        <p className="gl-desc">{t('schedule.desc')}</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '2.5rem' }}>
        ...
        {activeDays.map((day) => (
          ...
          >
            ...
            <span style={{ marginLeft: '0.4rem', background: selectedDay === day ? 'rgba(255,255,255,0.25)' : 'rgba(236,72,153,0.15)', color: selectedDay === day ? 'white' : 'var(--pink-accent)', borderRadius: '999px', fontSize: '0.7rem', padding: '0 0.4rem', fontWeight: 700 }}>
        ...
        <>
          {selectedDay === 'all' ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
              {activeDays.map((day) => (
                <DaySection key={day} day={day} translatedDay={translateDay(day)} movies={groupedByDay[day]} onSelect={setSelectedMovie} t={t} />
              ))}
            </div>
          ) : (
            <DaySection day={selectedDay} translatedDay={translateDay(selectedDay)} movies={displayMovies} onSelect={setSelectedMovie} hideHeader t={t} />
          )}
...
const DaySection = ({ day, translatedDay, movies, onSelect, hideHeader, t }) => {
  if (movies.length === 0) return null;
  return (
    <div>
      {!hideHeader && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
          <div style={{ width: 4, height: 28, borderRadius: 4, background: day === 'N/A' ? 'var(--text-muted)' : 'var(--pink-accent)' }} />
          <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.4rem', color: day === 'N/A' ? 'var(--text-muted)' : 'var(--text-main)' }}>
            {translatedDay}
          </h2>
          <span style={{ background: 'rgba(236,72,153,0.12)', color: 'var(--pink-accent)', fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '999px' }}>
            {movies.length} {t('schedule.movies_count')}
          </span>
        </div>
      )}
...
const ScheduleRow = ({ movie, onSelect }) => {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateX(4px)'; e.currentTarget.style.borderColor = 'var(--pink-accent)'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(236,72,153,0.15)'; }}
    ...
    >
      ...
      <div style={{ minWidth: 60, textAlign: 'center', flexShrink: 0 }}>
        <div style={{ background: time === 'N/A' ? 'var(--item-bg)' : 'rgba(236,72,153,0.12)', color: time === 'N/A' ? 'var(--text-muted)' : 'var(--pink-accent)', borderRadius: '0.5rem', padding: '0.25rem 0.5rem', fontSize: '0.8rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.25rem' }}>
          {time === 'N/A' ? <HelpCircle size={12} /> : <Clock size={12} />} {time}
        </div>
      </div>

      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: '0.975rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '0.2rem' }}>{movie.title}</div>
          ...
          <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
            ...
            {movie.youtube_url && (
              <a href={movie.youtube_url} target="_blank" rel="noreferrer" style={{ position: 'absolute', bottom: '1.5rem', right: '1.5rem', background: 'var(--pink-accent)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '2rem', display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none', fontWeight: 'bold', boxShadow: '0 4px 15px rgba(236,72,153,0.4)' }}>
                <PlayCircle size={20} /> {t('schedule.trailer')}
              </a>
            )}
          </div>
        )}
        <div style={{ padding: '2rem' }}>
          ...
          {movie.admin_note && (
            <div style={{ background: 'rgba(236,72,153,0.1)', borderLeft: '4px solid var(--pink-accent)', padding: '1rem', borderRadius: '0 0.5rem 0.5rem 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--pink-accent)', fontWeight: 'bold', marginBottom: '0.5rem' }}><Info size={18} /> {t('schedule.admin_note')}</div>
